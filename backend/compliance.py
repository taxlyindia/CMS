...
def _upsert(c,company_id,entity_type,entity_id,alert_type,title,message,due_date,severity):
    # Check for any existing alert for this entity — active, dismissed, or resolved
    c.execute("SELECT id,status FROM alerts WHERE entity_type=%s AND entity_id=%s AND alert_type=%s ORDER BY created_at DESC LIMIT 1",
              (entity_type,entity_id,alert_type))
    ex = c.fetchone()
    if ex:
        ex_id     = ex['id']     if isinstance(ex,dict) else ex[0]
        ex_status = ex['status'] if isinstance(ex,dict) else ex[1]
        if ex_status == 'active':
            # Update existing active alert
            c.execute("UPDATE alerts SET severity=%s,message=%s,due_date=%s WHERE id=%s",
                      (severity,message,due_date,ex_id))
        elif ex_status in ('dismissed','resolved'):
            # User manually dismissed/resolved — respect that, do NOT recreate
            pass
    else:
...
def generate_company_master_pdf(company_id: str) -> bytes:
    ...
    _co_pan = co.get("pan") or ""; _co_cin = co.get("cin") or ""
    _co_email = co.get("email") or ""; _co_phone = co.get("phone") or ""
    lh_footer = co.get("letterhead_footer") or " | ".join(p for p in [
        f"CIN: {_co_cin}"   if _co_cin   else "",
        f"PAN: {_co_pan}"   if _co_pan   else "",
        f"✉ {_co_email}" if _co_email else "",
        f"☎ {_co_phone}" if _co_phone else "",
    ] if p)

    # Letterhead header — clean multi-line format
    story.append(Spacer(1,4))
    story.append(Paragraph(co["name"].upper(), sty("h1",17,True,NAVY,TA_CENTER,22)))
    if lh_addr:
        addr_lines = [l.strip() for l in lh_addr.replace(" | ","|").split("|") if l.strip()]
        if not addr_lines: addr_lines = [lh_addr]
        for al in addr_lines:
            story.append(Paragraph(al, sty("addr_l",7.5,False,GREY,TA_CENTER,10)))
    _cm_email = co.get("email") or ""; _cm_phone = co.get("phone") or ""
    _cm_contact = "  |  ".join(p for p in [
        (f"✉ {_cm_email}" if _cm_email else ""),
        (f"☎ {_cm_phone}" if _cm_phone else ""),
    ] if p)
    if _cm_contact:
        story.append(Paragraph(_cm_contact, sty("lc",7.5,False,GREY,TA_CENTER,10)))
    story.append(Spacer(1,5))
    story.append(HRFlowable(width="100%",thickness=2,color=BLUE,spaceAfter=1))
    story.append(HRFlowable(width="100%",thickness=0.5,color=NAVY,spaceAfter=8))
    story.append(Paragraph("COMPANY MASTER DATA", sty("title",12,True,BLUE,TA_CENTER)))
    story.append(Paragraph(f"Generated on {date.today().strftime('%d %B %Y')}", sty("dt",7.5,False,GREY,TA_CENTER)))
...
def generate_register_pdf(company_id: str, register_type: str) -> bytes:
    ...
    story.append(Paragraph(co["name"].upper(), sty("co",15,True,NAVY,TA_CENTER)))
    if lh_addr:
    ...
    _reg_email = co.get("email") or ""; _reg_phone = co.get("phone") or ""
    ...
    for i, dr in enumerate(data_rows):
        ...
        row_vals = all_vals[:n_cols]  # Trim to match header count
        def _cell_str(v):
            if v is None: return "—"
        ...
        tdata.append([Paragraph(_cell_str(v), sty("d",7.5)) for v in row_vals])
    ...
    story.append(HRFlowable(width="100%",thickness=0.5,color=colors.HexColor("#94a3b8")))
    story.append(Spacer(1,3))
    _r_pan = co.get("pan") or ""; _r_cin = co.get("cin") or ""; _r_off = co.get("registered_office") or ""
    lh_footer = co.get("letterhead_footer") or " | ".join(p for p in [
        f"CIN: {_r_cin}" if _r_cin else "",
