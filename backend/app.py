...
def login():
    ...
    if not is_pa and user.get("tenant_id"):
        ...
        t = c.fetchone()
        if t and (t['status'] if isinstance(t,dict) else t[0]) not in ("active",):
...
def create_director():
    ...
    c.execute("INSERT INTO director_kyc (id,director_id,last_kyc_date,next_due_date,kyc_status) VALUES (%s,%s,%s,%s,%s)",
        ...
        for ri, row in enumerate(rows_data, start=5):
            ...
            try:
                if module == "companies":
                    ...
                    c.execute("INSERT INTO director_kyc (id,director_id,next_due_date,kyc_status) VALUES (%s,%s,%s,%s)",
...
def get_user_permissions(uid):
    ...
    overrides = [(dict(r) if isinstance(r,dict) else dict(zip([d[0] for d in c.description],r))) for r in c.fetchall()]
    conn.close()
    return jsonify({
        "user": user,
        "overrides": overrides,
