...
@login_required
def list_persons():
    """List all persons in directory with their company roles."""
    conn = get_db(); c = conn.cursor()
    tid = g.tenant_id
    q = """
        SELECT p.*,
            (SELECT COUNT(*) FROM directors d WHERE d.person_id=p.id AND d.is_active=1) AS director_count,
            (SELECT COUNT(*) FROM shareholders s WHERE s.person_id=p.id AND s.is_active=1) AS shareholder_count
        FROM person_directory p
    ...
    """ if tid else """
        SELECT p.*,
            (SELECT COUNT(*) FROM directors d WHERE d.person_id=p.id AND d.is_active=1) AS director_count,
            (SELECT COUNT(*) FROM shareholders s WHERE s.person_id=p.id AND s.is_active=1) AS shareholder_count
        FROM person_directory p ORDER BY p.name
    """
...
@app.route("/api/persons/<pid>/companies")
@login_required
def person_companies(pid):
    """All companies where a person is a director or shareholder."""
    conn = get_db(); c = conn.cursor()
    # Director roles
    c.execute("""
        SELECT co.id AS company_id, co.name AS company_name, co.cin,
               d.designation, d.date_of_appointment, d.date_of_cessation,
               d.din, d.pan, d.id AS director_id,
               'Director' AS role_type,
               CASE d.is_active WHEN 1 THEN 'Active' ELSE 'Resigned' END AS status
        FROM directors d
        JOIN companies co ON d.company_id = co.id
        WHERE d.person_id = %s
        ORDER BY co.name
    """, (pid,))
    director_roles = rows(c.fetchall())
    # Shareholder roles
    c.execute("""
        SELECT co.id AS company_id, co.name AS company_name, co.cin,
               s.share_class, s.shares_held, s.face_value,
               s.folio_no, s.pan, s.id AS shareholder_id,
               'Shareholder' AS role_type,
               CASE s.is_active WHEN 1 THEN 'Active' ELSE 'Exited' END AS status
        FROM shareholders s
        JOIN companies co ON s.company_id = co.id
        WHERE s.person_id = %s
        ORDER BY co.name
    """, (pid,))
    shareholder_roles = rows(c.fetchall())
    # Person details
    c.execute("SELECT * FROM person_directory WHERE id=%s", (pid,))
    person = row(c.fetchone())
    conn.close()
    if not person: return jsonify({"error": "Person not found"}), 404
    return jsonify({
        "person": person,
        "director_roles": director_roles,
        "shareholder_roles": shareholder_roles,
        "total_companies": len(set(
            [r["company_id"] for r in director_roles] +
            [r["company_id"] for r in shareholder_roles]
        ))
    })

@app.route("/api/persons/<pid>/companies/excel")
@login_required
def person_companies_excel(pid):
    """Download Excel report of all companies for a person."""
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT * FROM person_directory WHERE id=%s", (pid,))
    person = row(c.fetchone())
    if not person: conn.close(); return jsonify({"error": "Person not found"}), 404

    c.execute("""
        SELECT co.name AS company_name, co.cin,
               d.designation, d.din, d.pan,
        ...
        UNION ALL
        SELECT co.name AS company_name, co.cin,
               s.share_class AS designation, s.folio_no AS din, s.pan,
               s.date_of_entry AS date_of_appointment, NULL AS date_of_cessation,
               CASE s.is_active WHEN 1 THEN 'Active' ELSE 'Exited' END AS status,
               'Shareholder' AS role_type
        FROM shareholders s JOIN companies co ON s.company_id=co.id
        WHERE s.person_id=%s
        ORDER BY company_name
    """, (pid, pid))
    records = rows(c.fetchall())
    conn.close()

    import io as _io, xlsxwriter as _xl
    buf = _io.BytesIO()
    wb = _xl.Workbook(buf, {'in_memory': True})
    ws = wb.add_worksheet("Company Roles")
    NAVY = '#0f2d5c'; BLUE = '#1a56db'
    h_fmt  = wb.add_format({'bold':True,'font_color':'#ffffff','bg_color':NAVY,'border':1,'font_size':9})
    d_fmt  = wb.add_format({'font_size':9,'border':1,'valign':'vcenter'})
    t_fmt  = wb.add_format({'bold':True,'font_color':BLUE,'font_size':13})
    s_fmt  = wb.add_format({'italic':True,'font_color':'#64748b','font_size':9})

    ws.merge_range('A1:H1', f"Company Roles — {person['name']}", t_fmt)
    ws.write('A2', f"PAN: {person.get('pan') or '—'}   DIN: {person.get('din') or '—'}   "
             f"Mobile: {person.get('mobile') or '—'}   Email: {person.get('email') or '—'}", s_fmt)
    ws.write('A3', f"Total records: {len(records)}", s_fmt)

    headers = ["Company Name","CIN","Role Type","Designation/Share Class","DIN/Folio","PAN","Appointment Date","Status"]
    for ci, h in enumerate(headers):
        ws.write(3, ci, h, h_fmt)

    col_map = ["company_name","cin","role_type","designation","din","pan","date_of_appointment","status"]
    for ri, rec in enumerate(records):
        for ci, key in enumerate(col_map):
            val = rec.get(key, "") or ""
            ws.write(ri+4, ci, str(val) if val else "—", d_fmt)

    ws.set_column(0, 0, 30); ws.set_column(1, 1, 22); ws.set_column(2, 2, 12)
    ws.set_column(3, 3, 22); ws.set_column(4, 5, 15); ws.set_column(6, 6, 16)
    ws.set_column(7, 7, 10)
    wb.close(); buf.seek(0)
    fname = f"{person['name'].replace(' ','_')}_Company_Roles.xlsx"
...
@login_required
def create_person():
    """Manually create a person in directory."""
    d = request.get_json(silent=True, force=True) or {}
    if not d.get("name"): return jsonify({"error": "Name required"}), 400
    conn = get_db(); c = conn.cursor()
    pid = _get_or_create_person(c, d["name"],
                                 (d.get("pan") or "").upper() or None,
                                 d.get("din"),
                                 d.get("email"), d.get("mobile"),
                                 d.get("aadhaar"), d.get("address"), g.tenant_id)
    conn.commit()
    c.execute("SELECT * FROM person_directory WHERE id=%s", (pid,))
    result = row(c.fetchone()); conn.close()
    return jsonify(result), 201
...
def update_person(pid):
    """Update person directory entry — syncs to all director/shareholder records."""
    d = request.get_json(silent=True, force=True) or {}
    conn = get_db(); c = conn.cursor()
    fields = {k: d[k] for k in ["name","pan","din","email","mobile","aadhaar","address"] if k in d}
    if not fields: conn.close(); return jsonify({"error": "Nothing to update"}), 400
    if "pan" in fields: fields["pan"] = _str((fields["pan"] or "").upper())
    fields["updated_at"] = datetime.utcnow().isoformat()
    c.execute(f"UPDATE person_directory SET {','.join(k+'=%s' for k in fields)} WHERE id=%s",
              list(fields.values()) + [pid])
    # Sync name to all director/shareholder records
    if "name" in fields:
        c.execute("UPDATE directors SET name=%s WHERE person_id=%s", (fields["name"], pid))
        c.execute("UPDATE shareholders SET name=%s WHERE person_id=%s", (fields["name"], pid))
    conn.commit()
    c.execute("SELECT * FROM person_directory WHERE id=%s", (pid,))
    result = row(c.fetchone()); conn.close()
    return jsonify(result)
...
@login_required
def sync_person_directory():
    """Backfill person_directory from existing directors and shareholders."""
    conn=get_db(); c=conn.cursor()
    tid = g.tenant_id
    synced = 0
    # Sync directors missing person_id
    c.execute("SELECT * FROM directors WHERE person_id IS NULL AND is_active=1" +
              (" AND tenant_id=%s" if tid else ""), (tid,) if tid else ())
    for d in rows(c.fetchall()):
        try:
            pid = _get_or_create_person(c, d['name'],
                (d.get('pan') or '').upper() or None, d.get('din'),
                d.get('email'), d.get('mobile'), d.get('aadhaar'), d.get('address'),
                d.get('tenant_id'))
            c.execute("UPDATE directors SET person_id=%s WHERE id=%s", (pid, d['id']))
            synced += 1
        except Exception: pass
    # Sync shareholders missing person_id
    c.execute("SELECT * FROM shareholders WHERE person_id IS NULL AND is_active=1" +
              (" AND tenant_id=%s" if tid else ""), (tid,) if tid else ())
    for s in rows(c.fetchall()):
        try:
            pid = _get_or_create_person(c, s['name'],
                (s.get('pan') or '').upper() or None, None,
                s.get('email'), s.get('mobile'), None, s.get('address'),
                s.get('tenant_id'))
            c.execute("UPDATE shareholders SET person_id=%s WHERE id=%s", (pid, s['id']))
            synced += 1
        except Exception: pass
    conn.commit(); conn.close()
...
def all_directors():
    """
    Returns UNIQUE directors (one row per person, grouped by DIN).
    Each row includes company_count and companies list.
    When person_id is available, groups by that; else groups by DIN.
    """
    conn=get_db(); c=conn.cursor()
    tid = g.tenant_id
    ...
    c.execute(f"""
        SELECT
            COALESCE(d.person_id, d.din, d.id) AS group_key,
            d.name, d.din, d.pan, d.mobile, d.email, d.address,
            d.aadhaar, d.mca_user_id, d.mca_password, d.mca_notes,
            d.designation,
            MIN(d.id) AS id,
            MIN(d.company_id) AS company_id,
            d.person_id,
            MAX(k.kyc_status) AS kyc_status,
            MAX(k.next_due_date::TEXT) AS next_due_date,
            MAX(k.last_kyc_date::TEXT) AS last_kyc_date,
            COUNT(d.id) AS company_count,
            STRING_AGG(DISTINCT co.name, ' | ' ORDER BY co.name) AS companies,
            STRING_AGG(DISTINCT co.id::TEXT, ',' ORDER BY co.name) AS company_ids
        FROM directors d
        LEFT JOIN director_kyc k ON d.id=k.director_id
        LEFT JOIN companies co ON d.company_id=co.id
        WHERE d.is_active=1 {_tf}
        GROUP BY COALESCE(d.person_id, d.din, d.id), d.name, d.din, d.pan,
                 d.mobile, d.email, d.address, d.aadhaar, d.mca_user_id,
                 d.mca_password, d.mca_notes, d.designation, d.person_id
        ORDER BY d.name
    """, _tp)
    result = rows(c.fetchall())
    conn.close()
...
def search_directors():
    ...
    c.execute("""
        ...
        WHERE d.is_active=1 AND (
            LOWER(d.name) LIKE %s OR LOWER(d.din) LIKE %s OR LOWER(d.pan) LIKE %s
        ) ORDER BY d.name
    """, (f'%{q}%', f'%{q}%', f'%{q}%'))
    return jsonify(rows(c.fetchall()))
