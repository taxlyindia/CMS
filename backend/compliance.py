...
def run_compliance_checks():
    ...
    for dr in rows(c.fetchall()):
        ...
        c.execute("UPDATE director_kyc SET kyc_status=? WHERE director_id=?",
    ...
    c.execute("""SELECT m.*,co.name as company_name FROM meetings m
                 ...
                 WHERE m.status='scheduled' AND m.meeting_date < ?""",(today.isoformat(),))
...
def _upsert(c,company_id,entity_type,entity_id,alert_type,title,message,due_date,severity):
    c.execute("SELECT id FROM alerts WHERE entity_type=? AND entity_id=? AND alert_type=? AND status='active'",
              (entity_type,entity_id,alert_type))
    ex = c.fetchone()
    if ex:
        c.execute("UPDATE alerts SET severity=?,message=?,due_date=? WHERE id=?",(severity,message,due_date,ex[0]))
    else:
        c.execute("INSERT INTO alerts (id,company_id,entity_type,entity_id,alert_type,title,message,due_date,severity) VALUES (?,?,?,?,?,?,?,?,?)",
...
def get_active_entities(company_id: str) -> dict:
    ...
    c.execute("""
        ...
        WHERE d.company_id=?
          AND d.is_active=1
          AND (d.date_of_cessation IS NULL OR d.date_of_cessation='' OR d.date_of_cessation > date('now'))
...
def generate_company_master_pdf(company_id: str) -> bytes:
    ...
    c.execute("""SELECT d.*,k.last_kyc_date,k.next_due_date,k.kyc_status
                 ...
                 WHERE d.company_id=? ORDER BY d.date_of_appointment""", (company_id,))
    dirs = rows(c.fetchall())
    c.execute("SELECT * FROM auditors WHERE company_id=? ORDER BY created_at DESC", (company_id,))
    auds = rows(c.fetchall())
    c.execute("SELECT * FROM shareholders WHERE company_id=? AND is_active=1 ORDER BY folio_no", (company_id,))
    shs = rows(c.fetchall())
    c.execute("SELECT * FROM dsc_records WHERE company_id=? AND is_active=1", (company_id,))
    ...
    if dscs:
        ...
        def dsc_status(valid_to):
            ...
            d = date.fromisoformat(valid_to[:10])
            diff = (d-today).days
...
REGISTER_DEFINITIONS = {
    "MGT-1": {
        ...
        "query": "SELECT folio_no,name,pan,address,share_class,shares_held,face_value,date_of_entry,mobile,email FROM shareholders WHERE company_id=? AND is_active=1 ORDER BY folio_no",
    },
    "MGT-2": {
        ...
        "query": "SELECT folio_no,name,pan,share_class,shares_held,face_value,date_of_entry,mobile,email FROM shareholders WHERE company_id=? AND share_class='Debenture' ORDER BY folio_no",
    },
    "DIR-3": {
        ...
        "query": """SELECT d.name,d.din,d.pan,d.designation,d.date_of_appointment,d.date_of_cessation,
                    ...
                    FROM directors d LEFT JOIN director_kyc k ON d.id=k.director_id
                    WHERE d.company_id=? ORDER BY d.date_of_appointment""",
    },
    "MBP-1": {
        ...
        "query": "SELECT director_name,din,entity_name,entity_type,nature_of_interest,date_of_disclosure,date_of_board_resolution,remarks FROM director_interests WHERE company_id=? ORDER BY date_of_disclosure",
    },
    "ADT-1": {
        ...
        "query": "SELECT firm_name,membership_no,frn,pan,email,nature_of_appointment,appointment_type,start_date,end_date,srn_adt1 FROM auditors WHERE company_id=? ORDER BY start_date",
    },
    "CHG-1": {
        ...
        "query": "SELECT charge_id,charge_type,charge_holder,assets_charged,amount,date_of_creation,date_of_modification,date_of_satisfaction,status,remarks FROM charges WHERE company_id=? ORDER BY date_of_creation",
    },
    "SH-6": {
        ...
        "query": "SELECT employee_name,designation,employee_id,grant_date,options_granted,exercise_price,vesting_date,vesting_period,options_exercised,options_lapsed,status FROM esop_grants WHERE company_id=? ORDER BY grant_date",
    },
    "MBP-3": {
        ...
        "query": "SELECT investee_name,investee_type,investment_type,amount,date_of_investment,board_resolution_date,srn_mgb4,purpose,remarks FROM investments WHERE company_id=? ORDER BY date_of_investment",
    },
    "MBP-2": {
        ...
        "query": "SELECT party_name,party_type,transaction_type,amount,date_of_transaction,rate_of_interest,repayment_date,security,board_resolution_date,outstanding_amount,status FROM loans_guarantees WHERE company_id=? ORDER BY date_of_transaction",
    },
    "RPT-188": {
        ...
        "query": "SELECT party_name,relationship,nature_of_transaction,amount,date_of_transaction,date_of_board_approval,date_of_shareholders_approval,terms,remarks FROM related_party_transactions WHERE company_id=? ORDER BY date_of_transaction",
    },
    "DSC": {
        ...
        "query": "SELECT holder_name,holder_type,dsc_class,issued_by,valid_from,valid_to,token_type,custody_status,custody_date,notes FROM dsc_records WHERE company_id=? AND is_active=1",
    },
}
...
def generate_register_pdf(company_id: str, register_type: str) -> bytes:
    ...
    for i, dr in enumerate(data_rows):
        tdata.append([Paragraph(str(v or "—"), sty("d",7.5)) for v in dr])
