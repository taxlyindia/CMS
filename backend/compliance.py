...
def _upsert(c,company_id,entity_type,entity_id,alert_type,title,message,due_date,severity):
    ...
    ex = c.fetchone()
    if ex:
        c.execute("UPDATE alerts SET severity=%s,message=%s,due_date=%s WHERE id=%s",(severity,message,due_date,ex[0]))
    else:
...
def get_active_entities(company_id: str) -> dict:
    ...
    c.execute("""
        ...
        WHERE d.company_id=%s
          AND d.is_active=1
          AND (d.date_of_cessation IS NULL OR d.date_of_cessation='' OR d.date_of_cessation > CURRENT_DATE)
