...
def hash_pw(pw):
    """Hash password — SHA-256 for now, bcrypt-ready via verify_pw."""
    return hashlib.sha256(pw.encode()).hexdigest()

def verify_pw(plain: str, stored: str) -> bool:
    """Verify a password against its stored hash.
    Supports both SHA-256 (legacy) and bcrypt (if stored starts with $2b$).
    """
    ...
    if stored.startswith('$2b$') or stored.startswith('$2a$'):
        try:
            ...
            return False
    # SHA-256 hash (current default)
    import hashlib as _hl
    return _hl.sha256(plain.encode()).hexdigest() == stored
...
def load_user_permissions(user_id):
    try:
        ...
        c.execute("SELECT module, action, granted FROM user_permissions WHERE user_id=%s", (user_id,))
        overrides = {}
        for _r in c.fetchall():
            if isinstance(_r, dict):
                _vals = list(_r.values())
                overrides[(_vals[0], _vals[1])] = bool(_vals[2])
            else:
