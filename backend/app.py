...
# [BUG FIX] Duplicate 500 handler removed — using detailed handler below
...
def change_password():
    ...
    c.execute("UPDATE users SET password=%s WHERE id=%s",(auth_hash(new),g.user_id))
...
def create_user():
    ...
    try:
        ...
        c.execute(
            ...
            (uid, d["name"].strip(), d["email"].strip().lower(), auth_hash(d["password"]), role, phone_val, g.tenant_id)
        )
        conn.commit()
...
def reset_user_password(uid):
    ...
    if len(new_pw) < 6:
        return jsonify({"error": "Password must be at least 6 characters"}), 400
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE users SET password=%s WHERE id=%s", (auth_hash(new_pw), uid))
    conn.commit(); conn.close()
    return jsonify({"success": True})
...
def create_company():
    ...
    if len(d.get("name","")) > 200: return jsonify({"error":"Company name too long (max 200 chars)"}),400
    if d.get("cin") and len(d.get("cin","")) > 21: return jsonify({"error":"Invalid CIN format"}),400
    ...
    try:
     ...
     conn.commit()
        ...
        if 'unique' in msg.lower() or 'duplicate' in msg.lower():
            field = "CIN" if "cin" in msg.lower() else "PAN" if "pan" in msg.lower() else "identifier"
            return jsonify({"error": f"A company with this {field} already exists"}), 409
...
def create_director():
    ...
    if d.get("din") and not re.match(r'^\d{8}$', str(d.get("din","")).strip()): return jsonify({"error":"DIN must be exactly 8 digits"}),400
    if d.get("pan") and not re.match(r'^[A-Z]{5}[0-9]{4}[A-Z]$', str(d.get("pan","")).strip().upper()): return jsonify({"error":"Invalid PAN format (e.g. ABCDE1234F)"}),400
...
def create_meeting():
    ...
    if not d.get("company_id"): return jsonify({"error": "company_id is required"}), 400
    if not d.get("meeting_date"): return jsonify({"error": "meeting_date is required"}), 400
...
def update_meeting(mid):
    ...
    if d.get("status") not in (None,"scheduled","held","cancelled","postponed",""):
        return jsonify({"error": "Invalid status. Must be: scheduled, held, cancelled, or postponed"}), 400
...
def list_doc_templates():
    ...
    conn = get_db(); c = conn.cursor()
    try:
        q = """SELECT id,name,category,description,placeholders,is_system,is_active,created_at
               FROM document_templates
               WHERE is_active=1 AND (tenant_id=%s OR tenant_id='default-tenant-001' OR is_system=1)"""
        params = [g.tenant_id or 'default-tenant-001']
        if cat:
            q += " AND category=%s"; params.append(cat)
        ...
        app.logger.error(f"list_doc_templates error: {e}")
        return jsonify([])
    finally:
        conn.close()
...
def create_doc_template():
    ...
    conn.commit()
    write_audit_log(g.user_id, "template.create", "template", tid, d["name"], request.remote_addr, g.tenant_id)
...
def _startup():
    ...
    try:
        ...
        _add_col_safe(_mc, "shareholders", "shares_held_before_transfer", "INTEGER")
        _add_col_safe(_mc, "shareholders", "transfer_date_actual", "TEXT")
        _add_col_safe(_mc, "esign_requests", "company_id",    "TEXT")
        _add_col_safe(_mc, "esign_requests", "completed_at",  "TEXT")
        _add_col_safe(_mc, "esign_requests", "email_sent",    "INTEGER DEFAULT 0")
        _add_col_safe(_mc, "esign_requests", "wa_sent",       "INTEGER DEFAULT 0")
...
def resign_director(did):
    ...
    try:
        ...
        result = row(c.fetchone())
        conn.close()
        write_audit_log(g.user_id, "director_resign", "director", did,
                        f"date={resign_date} reason={reason}", request.remote_addr, g.tenant_id)
...
def ai_draft():
    ...
    co = {}
    if company_id:
        conn_ai = get_db(); c_ai = conn_ai.cursor()
        c_ai.execute("SELECT name,cin,company_type,registered_office,incorporation_date FROM companies WHERE id=%s AND tenant_id=%s", (company_id, g.tenant_id))
        co = row(c_ai.fetchone()) or {}
        conn_ai.close()
...
def portal_generate_link():
    ...
    write_audit_log(g.user_id, 'portal_link_created', 'portal', link_id,
                    f"token={token} company={data['company_id']} access={data.get('access_level')}",
                    request.remote_addr, g.tenant_id)
...
def wa_templates():
    if request.method == 'GET':
        try:
            _wc = get_db(); _wcc = _wc.cursor()
            _wcc.execute("SELECT * FROM wa_templates WHERE tenant_id=%s ORDER BY created_at DESC", (g.tenant_id,))
            recs = rows(_wcc.fetchall()); _wc.close()
...
def upload_word_template():
    ...
    result = row(c.fetchone()); conn.close()
    write_audit_log(g.user_id, "word_template_upload", "template", tid,
                    f"name={name} category={category}", request.remote_addr, g.tenant_id)
    return jsonify(result), 201
...
def send_wa_notification():
    ...
    write_audit_log(g.user_id, 'wa_send', 'notification', None,
...
# ══════════════════════════════════════════════════════════════════════════════
# PATCH: Missing endpoints added (wa-history, wa-template delete, portal fixes)
# ══════════════════════════════════════════════════════════════════════════════

@app.route("/api/notifications/wa-history")
@login_required
def get_wa_send_history():
    """WhatsApp send history for the tenant."""
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("""CREATE TABLE IF NOT EXISTS wa_send_log (
            id TEXT PRIMARY KEY, tenant_id TEXT, phone TEXT, message TEXT,
            type TEXT DEFAULT 'custom', status TEXT DEFAULT 'sent',
            sent_at TEXT, sent_by TEXT
        )""")
        conn.commit()
        c.execute("SELECT * FROM wa_send_log WHERE tenant_id=%s ORDER BY sent_at DESC LIMIT 100",
                  (g.tenant_id,))
        result = rows(c.fetchall()) or []
        return jsonify({"messages": result})
    except Exception as e:
        app.logger.error(f"wa_history error: {e}")
        return jsonify({"messages": []})
    finally:
        conn.close()
...
@app.route("/api/notifications/wa-templates/<tid>", methods=["DELETE"])
@login_required
def delete_wa_tpl_by_id(tid):
    """Delete a WhatsApp template."""
    conn = get_db(); c = conn.cursor()
    try:
        c.execute("DELETE FROM wa_templates WHERE id=%s AND tenant_id=%s", (tid, g.tenant_id))
        conn.commit()
        return jsonify({"success": c.rowcount > 0})
    except Exception as e:
        return jsonify({"error": str(e)}), 400
    finally:
        conn.close()
